...
const getUserLike = async (data:{
  ...
  if(type == "1"){
    ...
    LIKE '${params}%' OR u.email LIKE '${params}%' OR u.phone LIKE '${params}%' OR u.cedula LIKE '${params}%') and pu.userId = ${doctorId} and order by u.firstname asc `)
  }
  else{   
    ...
    inner join patient p on u.id = p.userId inner join patient_doctors_user pu on p.id = pu.patientId WHERE (concat(u.firstname,' ',u.lastname) 
    LIKE '${params}%' OR u.email LIKE '${params}%' OR u.phone LIKE '${params}%' OR u.cedula LIKE '${params}%') GROUP BY p.userId and order by u.firstname `)
